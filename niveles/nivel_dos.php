    ...
    <form class="multiplecolumn" method="post" id="guardar" action="" width="80%">
        ...
        <fieldset style="background-color:#ffffff00; width:30% ;text-align: center; ">
            ...
            <table border="0">
                ...
                $estado = array();
               $verifica=0;
                $q_nivel = mysqli_query($conexion_bd, "SELECT * FROM usuario WHERE referido='$_SESSION[username]' and nivel>'$nivel' ");
                while ($linea = mysqli_fetch_array($q_nivel)) {
                    $codigo[]=$linea['codigo'];                                                          
                    $username1[]=$linea['username'];                                                          
                ...
                if($verifica)
                {
                    ...
                    {
                        $q_nivel = mysqli_query($conexion_bd, "SELECT * FROM usuario WHERE username='$user'");
                        while ($linea = mysqli_fetch_array($q_nivel)) {
                            $nivel=$linea['nivel'];                                                          
                        }
                        $codigo = array();
                        ...
                        $estado = array();
                        $verifica=0;
                        $q_nivel = mysqli_query($conexion_bd, "SELECT * FROM usuario WHERE referido='$user' and nivel>'$nivel' ");
                        while ($linea = mysqli_fetch_array($q_nivel)) {
                            $codigo[]=$linea['codigo'];                                                          
                            $username1[]=$linea['username'];                                                          
                            $referido[]=$linea['referido'];                                                          
                        ...
                        if($verifica)
                        {
                            ...
                            foreach($username1 as $user)
                            {
                                ?>
                                <tr>
                                    <td ><input type="button" class="button" name="primer_nivel" value="<?php echo $referido[$i]; ?>" /></td>
                                    <td ><img src="../images/flecha-amarilla.png" width="80"  height="45"></td>
                                    <td ><input type="button" class="button" name="primer_nivel" value="<?php echo $username1[$i]; ?>" /></td>
               ...
               /* $nivel[]=$nivel+1;             
                ...
                do{
                    ...
                    $q_referido = mysqli_query($conexion_bd, "SELECT * FROM usuario WHERE referido='$referido' and nivel='$nivel'");
                    while ($referido = mysqli_fetch_array($q_referido)) 
                    {
                        $referido[]=$referido['username'];
